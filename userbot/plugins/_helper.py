...
async def yardim(event):
    ...
    if tgbotusername is not None or AuraX_input == "text":
        results = await event.client.inline_query(tgbotusername, "@Lucifer_support_group")
...
async def info(event):
    ...
    if input_str == "text":
        ...
        if len(string) > 4095:
            ...
            reply_text = f"All commands of the LuciferXBot are [here]({url})"
    ...
    if input_str:
        if input_str in SUDO_LIST:
            ...
            for i in SUDO_LIST[input_str]:
                ...
                AuraXcount += 1
            await event.reply(
                string.format(count=LuciferXcount, input_str=input_str), parse_mode="HTML"
            )
        else:
        ...
        for i in sorted(SUDO_LIST):
            ...
            string += " "
            AuraXcount += 1
        await event.reply(string.format(count=LuciferXcount), parse_mode="HTML")
