...
from userbot import *
from userbot import bot as LuciferXBot

DELETE_TIMEOUT = 5
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "LuciferX User"
AuraX_logo = "./LUCIFERX/LuciferXBot_logo.jpg"
aura = LuciferXBot.uid
AuraX = f"[{DEFAULTUSER}](tg://user?id={aura})"

@LuciferXBot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@LuciferXBot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    omk = f"**⍟ Plugin name ≈** `{input_str}`\n**⍟ Uploaded by ≈** {AuraX}\n\n⚡ **[Legendary LuciferXBot](t.me/Lucifer_support_group)** ⚡"
...
@LuciferXBot.on(admin_cmd(pattern="install$", outgoing=True))
@LuciferXBot.on(sudo_cmd(pattern="install$", allow_sudo=True))
                ...
                if shortname in CMD_LIST:
                    ...
                    for i in CMD_LIST[shortname]:
                        ...
                        if b == 1:
                            ...
                            b = 1
                        await event.edit(a)
                    return await event.edit(f"✅ **Installed module** :- `{shortname}` \n✨ BY :- {LuciferX}\n\n{string}\n\n        ⚡ **[Legendary LuciferXBot](t.me/Lucifer_support_group)** ⚡", link_preview=False)
...
@LuciferXBot.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@LuciferXBot.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
...
@LuciferXBot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
@LuciferXBot.on(sudo_cmd(pattern=r"upload (?P<shortname>\w+)$", allow_sudo=True))
...
@LuciferXBot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
@LuciferXBot.on(sudo_cmd(pattern=r"load (?P<shortname>\w+)$", allow_sudo=True))
...
CmdHelp("core").add_command(
  ...
  "cmds", None, "Gives out the list of modules in LuciferXBot."
).add()
